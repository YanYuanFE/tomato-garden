...
export class TomatoGardenService {
  ...
  async connectAccount(account: Account): Promise<void> {
    await Promise.all([
      this.nftService.connectAccount(account),
      this.stakingService.connectAccount(account)
    ]);
  }
   ...
   * 获取用户的NFT收藏详情
   */
  async getUserNFTCollection(userAddress: string): Promise<Array<{
    tokenId: string;
    owner: string;
    metadata: any;
    tokenUri: string;
  ...
  async getCollectionStats(userAddress: string): Promise<CollectionStats> {
    ...
    const collectedTypes = Object.values(userStats.tomatoesByType)
  ...
  private calculateTimeToNextStage(
    ...
    const GROWTH_TIME_PER_STAGE = 3600; // 1小时 - 调试模式
