...
import { TomatoMetadata, TomatoType, TransactionResult } from './types';
...
export class TomatoNFTService extends BaseContractService {
  ...
  async getTomatoMetadata(tokenId: string): Promise<TomatoMetadata> {
    const contract = await this.getNFTContract();
    const result = await this.executeCall<any>(contract, 'get_tomato_metadata', [tokenId]);
   ...
   * 批量获取NFT信息
   */
  async getBatchTokenInfo(tokenIds: string[]): Promise<Array<{
    tokenId: string;
    owner: string;
    metadata: TomatoMetadata;
    tokenUri: string;
  }>> {
    const batchCalls = tokenIds.map(tokenId => [
      {
   ...
   * 获取NFT交易历史
   */
  async getTokenHistory(tokenId: string): Promise<Array<{
    type: 'mint' | 'transfer';
    from: string;
    to: string;
