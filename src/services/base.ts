...
export class BaseContractService {
   ...
   * 加载合约实例
   */
  protected async loadContract(contractKey: keyof typeof this.network.contracts, abi: any[]): Promise<Contract> {
    ...
    const contract = new Contract(abi, contractAddress, this.account || this.provider);
